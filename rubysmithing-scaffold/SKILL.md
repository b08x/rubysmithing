...
description: Ruby project scaffolder using rubysmith (apps, tools, scripts) or gemsmith
  ...
  generate project skeleton, project template, set up a new ruby app, start a ruby tool,
  start a ruby script, create a new gem for rubygems, initialize ruby project,
  rubysmith build, gemsmith build, new ruby app, create a ruby tool. Runs rubysmith
  for local Ruby tools and apps; runs gemsmith for gems intended for rubygems.org.
  Executes the CLI directly, displays the generated file tree, then optionally applies
  Standard Mode convention hardening (frozen_string_literal, Zeitwerk, journald-logger,
  async boot). After scaffolding, suggests and chains to rubysmithing-tui,
...
**Note:** rubysmithing-context is NOT needed for this step. The scaffold CLI invokes
build toolchain gems, not runtime library APIs. Activate rubysmithing-context when
